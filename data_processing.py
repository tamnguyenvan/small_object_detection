...
def image_example(image_string, height, width, label, bbox):
    feature = {
...
def write_tfrecords(data, file_path, train_dir):
    with tf.io.TFRecordWriter(file_path) as writer:
        for img_info in tqdm(data):
            image_path = "{}/{:07d}.jpg".format(train_dir, img_info["id"])
...
class DataProcessing:
    def __init__(self, width=400, height=154, augment=True,
                 mix_iterator=None,convert_xywh=True,
    ...
    def random_crop(self, image, bbox, labels, origin_height, origin_width):
        width = self.width
        height = self.height
        ...
        origin_height = tf.cast(origin_height, tf.int32)
        origin_width = tf.cast(origin_width, tf.int32)
        if tf.greater(x1 + width, origin_width):
            x1 = origin_width - width

        if tf.greater(y1 + height, origin_height):
            y1 = origin_height - height
    ...
    def preprocess_data(self, example):
        ...
        """
        sample = tf.io.parse_example(example, image_feature_description)
        image = tf.io.decode_image(sample["image"], channels=3)
        bbox = tf.cast(
            tf.io.decode_raw(sample["bbox"], out_type=tf.int64), dtype=tf.float32
        )
        origin_height = tf.cast(sample['height'], tf.float32)
        origin_width = tf.cast(sample['width'], tf.float32)
        image = tf.reshape(image, (origin_height, origin_width, 3))
        ...
        if not self.augment:
            ...
            image = tf.image.resize(image, (origin_height, origin_width))
        ...
        if self.random_cropping and tf.logical_or(has_smallb, tf.random.uniform(()) > 0.5):
            image, bbox, label = self.random_crop(image, bbox, label, origin_height, origin_width)

        bbox = normalize_bbox(bbox, origin_width, origin_height)
